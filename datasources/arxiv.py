...
def process(arxiv_metadata_df, arxiv_storage_size=None):
    try:
        ...
        arxiv_fulltext_df["id"] = arxiv_fulltext_df["id"].str.replace("v\d+", "").str.replace(".txt", "")
        ...
        processed_arxiv_df["abstract"] = processed_arxiv_df["abstract"].str.replace(r'\s+[a-zA-Z]\s+', ' ').str.replace(r'\s+', ' ').str.strip()
